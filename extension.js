...
async function createExporterFilesRecursively(folder) {
    ...
    const filePath = path.join(folder, `${folderName}_src.dart`);
        ...
        if (isLikelyExportFile) {
            ...
            const overwriteChoice = userChoice
                ...
                : await vscode.window.showInformationMessage(
                    `Exporter file '${folderName}_src.dart' already exists in ${folder}. Do you want to overwrite it?`,
                    ...choices
                );
            ...
            vscode.window.showErrorMessage(
                `File '${folderName}_src.dart' already exists in ${folder}, and it may not be an export file. Skipping this folder.`
            );
            return;
        }
    }
...
function generateFileContent(folderName, subfolders, files) {
    const subfolderExports = subfolders
        .map((subfolder) => {
            ...
            return `export '${relativePath}/${path.basename(subfolder)}_src.dart';`;
        })
        .join("\n");
